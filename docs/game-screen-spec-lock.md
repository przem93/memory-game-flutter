...
- **Bottom ad strip (future):** When the AdMob bottom banner is integrated per `docs/admob-bottom-banner-spec-lock.md`, the available vertical space for the board **excludes** the fixed-height ad slot at the bottom of the safe area (see `docs/game-screen-layout-refinement-spec-lock.md` § “Future extension (AdMob bottom banner)”).
